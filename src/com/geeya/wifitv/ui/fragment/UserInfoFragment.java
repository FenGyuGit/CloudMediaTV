...
public class UserInfoFragment extends BaseFragment implements OnClickListener, ILogout {
	...
	public void onResume() {
		super.onResume();
		String area = null;
		area = application.getAreaInfo().getArea();
		if (area != null) {
			tvLocation.setText(area);
		}
		if (application.getUserInfo().getUserIdentify() == 1)
			btnLogin.setText(R.string.user_login);
		else {
			btnLogin.setText(R.string.user_logoff);
		}
		tvName.setText(application.getUserInfo().getUserName());
		String userFaceUrl = application.getUserInfo().getUserFace();
		if (userFaceUrl != null) {
			ImageLoader imageLoader = VolleyUtil.getImageLoader();
			imageLoader.get(userFaceUrl, ImageLoader.getImageListener(civFace, R.drawable.userinfo_headpicture, R.drawable.userinfo_headpicture));
		}
	};
	...
	private void initViewId() {
		...
		rlBg.setBackgroundResource(R.drawable.userinfo_source_background);
	...
	public void onClick(View view) {
		...
		case R.id.user_wealth:
			if (application.getUserInfo().getUserIdentify() == 0) {
				//Intent intent = new Intent(context, LoginActivity.class);
...
//			Intent intent = new Intent(context, BlankActivity.class);
		...
		case R.id.user_coupons:
			Intent intent5 = new Intent(context, CouponsActivity.class);
			//Intent intent5 = new Intent(context, BlankActivity.class);
			startActivity(intent5);
			break;
		case R.id.user_collection:
			Intent intent6 = new Intent(context, BlankActivity.class);
	...
	public void afterLogout() {
		tvName.setText("游客");
		btnLogin.setText("登陆");
		civFace.setImageDrawable(context.getResources().getDrawable(R.drawable.userinfo_headpicture));
	}
}
