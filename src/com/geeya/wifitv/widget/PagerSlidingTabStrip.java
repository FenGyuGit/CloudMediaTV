...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	//private Paint dividerPaint; //label中的分割线
	...
	private int tabPadding = 2;//原label大小减去padding之后，为文本或者图片在滑动栏中所能占有的大小
	private int dividerWidth = 1;

	private int tabTextSize = 15;
	private int tabTextColor = getResources().getColor(R.color.gray_text);
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		if(dm.widthPixels / dm.density <= 360){
			...
			defaultTabLayoutParams = new LinearLayout.LayoutParams(180, LayoutParams.MATCH_PARENT);
			...
			for(int i = 0; i < tabCount; i ++){
				...
				if(v1 instanceof TextView){
					...
					if(i == position)
						...
						if(tab.getCurrentTextColor() != tabTextColor){
							tab.setTextColor(tabTextColor);
						}
				}
			}
