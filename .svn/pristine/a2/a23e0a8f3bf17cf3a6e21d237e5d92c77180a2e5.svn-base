...
public class CornerADPlayer implements OnPauseListener {
	...
	public void setCornerADView(ArrayList<ADInfo> adInfoes) {
		...
		if (cornerADInfo != null) {
			...
			if (cornerADInfo.getADType().equals("image")) {
				...
				imageLoader.get(adInfoes.get(0).getADContent(), ImageLoader.getImageListener(corner, R.drawable.ic_empty, R.drawable.ic_empty));
				// 开启挂角广告和字幕广告开启线程
				if (myThread == null) {
	...
	public void setPauseADView(ArrayList<ADInfo> adInfoes) {
		...
		if (pauseADInfo != null) {
			if (pauseADInfo.getADType().equals("image")) {
				ImageLoader imageLoader = VolleyUtil.getImageLoader();
				imageLoader.get(adInfoes.get(0).getADContent(), ImageLoader.getImageListener(pause, R.drawable.ic_empty, R.drawable.ic_empty));
	...
	public void showCorner() {
		updateUserBehaviour.updateBrowseBehaviour(cornerADInfo.getADID());
	...
	public void onPauseShow() {
		// TODO Auto-generated method stub
		if (pauseADInfo != null) {
			Log.d("CornerADPlayer", "pauseADInfo.adID : " + pauseADInfo.getADID());
			updateUserBehaviour.updateBrowseBehaviour(pauseADInfo.getADID());
	...
	public static class ADPlayerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_CORNER_VISIBLE:
				if(cornerADPlayer.getStartThread())
