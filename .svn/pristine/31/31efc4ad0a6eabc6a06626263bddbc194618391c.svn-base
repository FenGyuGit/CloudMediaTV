		...
		if (WiFiTVApplication.getInstance().getNetworkCheck().checkNetWorkState()) {
			JsonObjectRequest req = new JsonObjectRequest(Method.GET, url, null, new Response.Listener<JSONObject>() {
				...
				public void onResponse(JSONObject jsonObject) {
					...
					//Log.i("VolleyUtil-GET", jsonObject.toString());
				...
				public void onErrorResponse(VolleyError volleyError) {
					// TODO Auto-generated method stub
					//Log.e("VolleyUtil-Get", "Volley-Error : " + volleyError.getMessage());
			...
			// if (entry == null || (entry != null && (entry.isExpired() ||
			// entry.refreshNeeded()))) {
			if (entry == null) {
				response.callback(null);
			} else {
