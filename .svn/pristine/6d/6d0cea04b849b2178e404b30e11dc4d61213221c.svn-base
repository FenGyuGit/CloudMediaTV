...
public class StartADPlayer implements OnPreparedListener, OnCompletionListener, OnBufferingUpdateListener, OnVideoSizeChangedListener, SurfaceHolder.Callback {
	...
	private UpdateUserBehaviour updateUserAction;
	...
	public static final int MSG_STOP_PLAY = 4;
	...
	public StartADPlayer(Context context, Activity activity, ArrayList<ADInfo> adInfoes) {
		...
		updateUserAction = new UpdateUserBehaviour();
	}
	...
	public OnADCompletionListener getOnADCompletionListener() {
		return onADCompletionListener;
	}
	...
	public void analyseADInfo(int index) {
		if (index < length) {
			...
			if (type.equals("video")) {
				// 开始视频广告的播放
				isUpdate = true;
				updateUserAction.updateBrowseBehaviour(adInfo.getADID());
				startVideoAD();
	...
	private void startImageAD(int adDuration) {
		...
		imageLoader.get(adContent, ImageLoader.getImageListener(imageView, R.drawable.ic_empty, R.drawable.ic_empty));
		isUpdate = true;
...
//		}
	...
	static class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_STOP_PLAY:
				adPlayer.surfaceView.setVisibility(View.GONE);
				adPlayer.tvTimeCount.setVisibility(View.GONE);
				adPlayer.releaseMediaPlayer();
				adPlayer.stopADPlayer();
				if(adPlayer.getOnADCompletionListener() != null)
					adPlayer.getOnADCompletionListener().onADCompletion();
				break;
			default:
				break;
			}
		...
		public void run() {
			while (isContinue) {
				...
				try {
					...
					Thread.sleep(1000);
					if(++waitTime > 5 && mediaPlayer != null && !mediaPlayer.isPlaying())
						myHandler.sendEmptyMessage(StartADPlayer.MSG_STOP_PLAY);
