			...
			public void callback(String str) {
				...
				if(listener != null){
					if(result == 0)
						...
						listener.onFailure(context.getString(R.string.login_failed_errresponding));
					}else if (2 == result) {
						listener.onFailure(context.getString(R.string.login_failed_noresponding));
					}else {
						listener.onFailure(context.getString(R.string.login_failed_errrequest));
					}
				}
			}
		});
		...
		if (null != data && (!data.equals(""))) {
			try {
				...
				if (flag) {
					...
					errorCode = dataAnalysis(content);
				} else {
		...
		if (null != data && (!data.equals(""))) {
			try {
				...
				if (!flag)
					...
					JSONObject content = response.getJSONObject("content");
					int err = dataAnalysis(content);
					if(err == 0)
						result = "注册成功";
					else if (err == 1) {
						result = context.getResources().getString(R.string.login_failed_errresponding);
					}
				}
			} catch (Exception e) {
				e.printStackTrace();
				result = context.getString(R.string.login_failed_errresponding);
			}
		}else {
			result = context.getResources().getString(R.string.login_failed_noresponding);
		}
		return result;
	}
	
	public int dataAnalysis(JSONObject content){
		int errorCode = 0;
		try {
			if (null != content && (!content.equals(""))) {
				int istelephone = content.toString().indexOf("telephone");
